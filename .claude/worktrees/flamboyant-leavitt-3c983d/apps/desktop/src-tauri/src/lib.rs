...
struct PullRequest {
    ...
    /// Logins of requested reviewers. GitHub team reviewers appear as `"team:<slug>"`.
...
fn parse_single_pr(json: &str) -> Result<PullRequest, String> {
    ...
    // Parse reviewRequests: [{requestedReviewer:{login:"..."}}, {requestedTeam:{slug:"..."}}]
    // Teams are stored as "team:<slug>" to distinguish them from user logins.
    ...
    if let Some(pos) = json.find("\"reviewRequests\"") {
        ...
        if let Some(arr_start) = rest.find('[') {
            if let Some(arr_end) = rest[arr_start..].find(']') {
                ...
                let mut search = 0;
                while let Some(spos) = arr[search..].find("\"slug\"") {
                    let abs = search + spos;
                    if let Some(v) = extract_json_string(&arr[abs..], "slug") {
                        if !v.is_empty() { review_requested.push(format!("team:{}", v)); }
                    }
                    search = abs + 6;
                }
            }
