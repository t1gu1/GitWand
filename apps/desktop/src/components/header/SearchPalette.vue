 ...
 *   - Any printable key → update query (native input behaviour)
 *
 * Data flow
 * ─────────
 * Palette is dumb: parent passes `branches`, `commits`, and a curated
 * `actions` array. Parent also handles every emitted event (switch
 * branch, navigate commit, run action id) — the palette never touches
 * git state directly.
 */
...
// ─── Filtering ───────────────────────────────────────────────────
// Empty query → show a curated default: the first N of each group.
// Non-empty → pure substring match, case-insensitive, across the item's
// relevant text (branch name / commit summary+hash / action label).
const DEFAULT_BRANCHES = 20;
const DEFAULT_COMMITS = 10;

const filteredBranches = computed<GitBranch[]>(() => {
  const q = query.value.trim().toLowerCase();
  // Skip the currently-checked-out branch: switching to the one you're
  // already on is a no-op, and surfacing it here just dilutes results.
...
/**
 * Flattened, typed index of everything currently visible. Selection
 * navigation operates on this list — rendering walks it via the
 * per-group arrays above but the keyboard index is a single number.
 */
type FlatItem =
...
// Keep selection clamped when the query changes — otherwise typing can
// leave selectedIndex pointing past the end of a shrunken list.
...
// ─── Selection offsets (for mouse hover + scroll-into-view) ─────
// Branches render first, then commits, then actions. We compute each
// group's starting offset in the flat list so the template can map
// per-group indices into the global selection scheme in O(1).
...
function activate(index: number) {
  ...
  if (!item) return;
  if (item.kind === "branch") {
    // For remote refs, strip the `origin/` prefix so the switchBranch
    // handler treats it as "check out a local copy of this remote".
...
function scrollSelectedIntoView() {
  // Wait for the .palette-item--active class to land on the right row
  // before measuring; otherwise we scroll the previously-selected row.
  nextTick(() => {
...
// ─── Lifecycle ──────────────────────────────────────────────────
onMounted(() => {
  // Autofocus the input — we keep it inside the modal so the native
  // `autofocus` attribute doesn't race the teleport mount.
  nextTick(() => inputEl.value?.focus());
  // Listen for keydown at the document level so Escape / arrow keys
  // work regardless of where focus sits inside the modal.
...
// Short hash (7 chars) — what git itself uses for display.
function shortHash(h: string): string {
  return h.slice(0, 7);
}

// "3h ago" relative label would be nice but requires a time lib;
// for MVP we just show the full date as provided by the log entry.
...
<template>
  <Teleport to="body">
    <div class="palette-overlay" role="dialog" aria-modal="true" @click.self="emit('close')">
...
.palette-modal {
  ...
  overflow: hidden;
  animation: modalSlide 0.18s cubic-bezier(0.16, 1, 0.3, 1);
}
